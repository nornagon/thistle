...
(function() {
  var cssColorToRGB, fmod, hslToCSS, hslToRGB, hueToRGB, knob, makeHSLCircle, makeHSLRef, makePicker, map, presentModalPicker, presentModalPickerBeneath, rgbToHSL, style,
    __slice = [].slice;
  ...
  makePicker = function(color) {
    var attachSaturationControl, circle, circleContainer, colorPreview, currentH, currentL, currentS, div, hsl, k, lKnob, lSlider, listeners, originalColor, originalColorTransparent, picker, radius, ref, rgb, setH, setL, setS, width;
    if (color == null) {
      color = {
        h: 180,
        s: 1,
    ...
    Object.defineProperty(picker, 'cssColor', {
      get: function() {
        return hslToCSS(currentH, currentS, currentL);
      },
      set: function(css) {
        return this.rgb = cssColorToRGB(css);
      }
    });
  ...
  presentModalPicker = function(x, y, color) {
    var modalFrame, picker;
    ...
    modalFrame.onclick = function() {
      ...
      picker.el.style.top = y + 10 + 'px';
      picker.el.style.opacity = 0;
      picker.el.addEventListener('webkitTransitionEnd', webkitEnd = function() {
        picker.el.parentNode.removeChild(picker.el);
        return picker.el.removeEventListener('webkitTransitionEnd', webkitEnd);
      });
      picker.el.addEventListener('transitionend', end = function() {
        document.body.removeChild(picker.el);
        return picker.el.removeEventListener('transitionend', end);
      });
      return picker.emit('closed');
    };
  ...
  presentModalPickerBeneath = function(el, color) {
    var elPos, x, y;
    elPos = el.getBoundingClientRect();
    x = elPos.left + window.scrollX;
    y = elPos.bottom + window.scrollY + 4;
    return presentModalPicker(x, y, color);
  };

  window.thistle = {
    makePicker: makePicker,
    presentModalPicker: presentModalPicker,
    presentModalPickerBeneath: presentModalPickerBeneath
  };
